...
const { fromBuffer: fileTypeFromBuffer } = pkg;
...
const reqContext = new AsyncLocalStorage<AccountContext>();
...
function createMcpServer(): McpServer {
  ...
  server.tool(
    'mint_moment',
    'Mint content permanently to the blockchain. Encrypts content based on privacy phase, pins to IPFS, and mints an ERC-721 NFT on Polygon. Returns a Block ID. Text/image mints cost 1 credit. Video mints (mp4, webm, mov) cost 2 credits and support files up to 50MB. Supports optional source metadata for provenance tracking (capture timestamp, platform, author, engagement, etc.).',
    {
  ...
  server.tool(
    'mint_from_url',
    'Mint a moment from a public URL. Hekkova fetches the content, extracts media and metadata, and mints it to the blockchain. Works with public social media posts, image URLs, and web pages. Accepts optional source metadata for provenance tracking (same schema as mint_moment).',
    {
      ...
      source: SourceMetadataSchema.optional().describe(
        'Optional provenance metadata. Same schema as mint_moment source parameter.'
      ),
    },
    async (input) => {
      const ctx = reqContext.getStore();
  ...
  server.tool(
    'update_phase',
    'Change the privacy phase of an existing moment. Phase Shifts cost credits: 1 credit for text/image moments, 2 credits for video moments. Legacy Plan subscribers get 10 free Phase Shifts per calendar month; additional shifts beyond 10 deduct credits normally.',
    {
  ...
  server.tool(
    'upload_media',
    'Upload a local file to Hekkova\'s temporary staging area. Returns a short-lived URL (valid 15 minutes) that you can pass to mint_from_url to mint the content. No mint credit is consumed. This is the recommended way to mint local images, video, and audio — do NOT use mint_moment for binary media.',
    {
    ...
    async (input) => {
      ...
      if (!rlResult.allowed) {
        ...
        const mapped = errorToMcpResponse(
          Object.assign(new Error(`Staging upload rate limit exceeded. Retry after ${retryAfter} seconds.`), { code: 'RATE_LIMITED' })
        );
        return { content: [{ type: 'text', text: JSON.stringify(mapped, null, 2) }], isError: true };
      }
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (event.type === 'customer.subscription.deleted') {
        const subscription = event.data.object as { metadata?: { account_id?: string } };
...
app.get('/staging/:key', async (req: Request, res: Response): Promise<void> => {
...
async function executeStagingUpload(
  fileBase64: string,
  filename: string,
  contentType: string,
  accountId: string
): Promise<{ upload_url: string; expires_in: number; content_type: string; size_bytes: number }> {
  if (!STAGING_UPLOAD_TYPES.includes(contentType as StagingContentType)) {
    throw Object.assign(
      new Error(`Unsupported content_type: ${contentType}. Allowed: ${STAGING_UPLOAD_TYPES.join(', ')}`),
...
app.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (req.file) {
      ...
      contentType = (req.body as Record<string, string>)['content_type'] ?? req.file.mimetype;
    } else {
      const body = req.body as { file?: string; filename?: string; content_type?: string };
    ...
    try {
      ...
      const e = err as Error & { code?: string };
      const code = e.code ?? 'INTERNAL_ERROR';
      const statusMap: Record<string, number> = { INVALID_MEDIA_TYPE: 400, MEDIA_TOO_LARGE: 413 };
...
app.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const blockId = req.params['block_id'] as string;
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (isMultipart) {
      ...
      if (!body.upload_url) {
        ...
        console.log(
          `[${new Date().toISOString()}] POST /api/mint (text) | account=${accountContext.account.id} | title="${title}" | phase=${phase}`
        );
