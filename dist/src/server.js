...
async function requireSupabaseAuth(authHeader) {
    ...
    if (email && account.light_id) {
        sendWelcomeEmail(email, account.display_name, account.light_id).catch((err) => {
...
app.patch('/api/moments/:block_id/phase', async (req, res) => {
    let account;
    try {
        account = await requireSupabaseAuth(req.headers.authorization);
    }
...
app.get('/api/phase-shifts/remaining', async (req, res) => {
    let account;
    try {
        account = await requireSupabaseAuth(req.headers.authorization);
    }
...
async function checkFilecoinDeals() {
    let pending;
    try {
        pending = await getMomentsWithPendingFilecoin();
    }
    catch (err) {
        console.error('[filecoin] Failed to query pending deals:', err.message);
        return;
    }
    if (pending.length === 0)
        return;
    ...
    for (const moment of pending) {
        ...
        try {
            ...
            console.error(`[filecoin] Error checking ${moment.block_id}:`, err.message);
        }
    }
}
