...
async function pbkdf2Key(input: Buffer | string, salt: Buffer): Promise<Buffer> {
...
function aesGcmEncryptStr(plaintext: string, key: Buffer): { ciphertext: string; iv: string } {
...
function aesGcmEncryptBytes(plaintext: Buffer, key: Buffer): { ciphertext: string; iv: string } {
  const iv = crypto.randomBytes(IV_BYTES);
  const cipher = crypto.createCipheriv('aes-256-gcm', key, iv);
  const ct = Buffer.concat([
...
async function generateEncryptedMoment(): Promise<void> {
...
function generateFullMoonMoment(): void {
...
async function generateImageMoment(): Promise<void> {
