...
export function bytesToBase64(bytes: Buffer | Uint8Array): string {
 ...
 */
export async function deriveKey(input: string | Buffer, salt: Buffer): Promise<Buffer> {
 ...
 */
export function computeVerificationHash(key: Buffer): string {
 ...
 */
export async function getMasterKey(ownerId: string): Promise<Buffer> {
  const masterSecret = config.serverMasterSecret;
  if (!masterSecret) {
    throw Object.assign(
      new Error('SERVER_MASTER_SECRET is not configured. Cannot encrypt moments.'),
  ...
  if (!encData) {
    throw Object.assign(
      new Error('Owner encryption data not found.'),
      { code: 'ENCRYPTION_DATA_NOT_FOUND' }
    );
  }

  const {
    server_encrypted_entropy,
    server_entropy_iv,
    server_entropy_salt,
    seed_salt,
  } = encData;
  ...
  if (!server_encrypted_entropy || !server_entropy_iv || !server_entropy_salt || !seed_salt) {
    throw Object.assign(
      new Error(
        'Passphrase setup incomplete. Please finish setup at app.hekkova.com before minting encrypted moments.'
      ),
      { code: 'PASSPHRASE_SETUP_INCOMPLETE' }
    );
  }
  ...
  const entropyBytes = await decryptBytesAESGCM(
    server_encrypted_entropy,
    server_entropy_iv,
    wrappingKey
  );
