...
export async function handleUpdatePhase(
  ...
  if (!moment) {
    const err = new Error(`No moment found with block_id: ${block_id}`) as Error & {
      code: string;
    };
  ...
  let newMediaCid: string | null = null;
  ...
  if (crossing && account.legacy_plan) {
    ...
    if (shouldEncrypt(targetPhase)) {
      const { encryptedData } = await encryptForPhase(
        moment.media_cid, // stub: we pass the CID as a stand-in for actual media
        targetPhase,
        accountContext
      );
      newMediaCid = await pinMedia(
        encryptedData,
        moment.media_type,
        `re_encrypted_${block_id}`
      );
    } else {
      // Decrypting to public — for the stub we just return a new fake CID
      newMediaCid = await pinMedia(
        moment.media_cid, // stub
        moment.media_type,
        `decrypted_${block_id}`
      );
    }
