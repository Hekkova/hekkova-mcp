...
async function uploadToLighthouse(buffer: Buffer): Promise<string | null> {
  try {
    ...
    console.warn('  [lighthouse] Upload error:', (err as Error).message);
    return null;
  }
}
...
async function main() {
  ...
  for (let i = 0; i < moments.length; i++) {
    const m = moments[i] as { block_id: string; media_cid: string; title: string };
    ...
    if (i < moments.length - 1) await sleep(DELAY_MS);
  }
