 ...
 */
export async function atomicMintDecrement(accountId, amount) {
    const supabase = getSupabase();
    ...
    if (error)
        throw new Error(`Credit decrement failed: ${error.message}`);
    if (!data) {
        const msg = amount === 2
 ...
 */
export async function decrementMintsBy(accountId, amount) {
    const supabase = getSupabase();
    ...
    if (error)
        throw new Error(`Credit decrement failed: ${error.message}`);
    if (!data) {
        throw Object.assign(new Error(`Insufficient credits. Purchase more at ${config.purchaseUrl}`), { code: 'INSUFFICIENT_BALANCE' });
    }
}
 ...
 */
export async function logPhaseShift(accountId, momentId) {
    const supabase = getSupabase();
    const { error } = await supabase
 ...
 */
export async function getPhaseShiftCount(accountId, monthStart, monthEnd) {
    const supabase = getSupabase();
