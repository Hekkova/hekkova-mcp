...
async function callTool(
  ...
  if (contentType.includes('text/event-stream')) {
    ...
    if (!dataLine) throw new Error('Empty SSE stream from MCP server');
    json = JSON.parse(dataLine.slice(5).trim()) as McpResponse;
  } else {
    json = (await res.json()) as McpResponse;
  }
...
function printResult(toolName: string, result: unknown, pass: boolean): void {
...
async function main(): Promise<void> {
  ...
  // 3. mint_moment
  const mintResult = await runTest(
    'mint_moment',
    {
  ...
  const blockId =
    ...
    'block_id' in mintResult
      ? (mintResult as { block_id: string }).block_id
      : '0x000000000000';
  ...
  await runTest(
    'update_phase',
    { block_id: blockId, new_phase: 'crescent' },
    (r) => {
      const res = r as Record<string, unknown>;
      return (
        typeof res.block_id === 'string' ||
