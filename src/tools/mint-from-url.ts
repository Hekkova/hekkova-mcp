...
function isDirectMediaUrl(_url: string, contentType: string): boolean {
  const base = contentType.split(';')[0].trim().toLowerCase();
 ...
 */
async function fetchWithSsrfGuard(targetUrl: string, userAgent: string, maxHops = 5): Promise<Response> {
  ...
  for (let hop = 0; hop < maxHops; hop++) {
    ...
    if (res.status >= 300 && res.status < 400) {
      ...
      if (!location) {
        const err = new Error('Redirect with no Location header') as Error & { code: string };
  ...
  const err = new Error('Too many redirects') as Error & { code: string };
  err.code = 'URL_FETCH_FAILED';
  throw err;
}
...
async function fetchAndExtract(url: string, titleOverride?: string): Promise<FetchedContent> {
  ...
  try {
    ...
    fetchErr.code = (err as Error & { code?: string }).code ?? 'URL_FETCH_FAILED';
    throw fetchErr;
  }
...
export async function handleMintFromUrl(
  ...
  if (stagingMatch) {
    ...
    if (!staging || new Date(staging.expires_at) < new Date()) {
      const err = new Error(
        'Staging upload has expired. Upload the file again with upload_media.'
      ) as Error & { code: string };
    ...
    if (staging.account_id !== accountContext.account.id) {
      const err = new Error('Staging upload not found') as Error & { code: string };
      err.code = 'NOT_FOUND';
      throw err;
    }
    ...
    try {
      ...
      const fetchErr = new Error(`Failed to fetch staged file: ${(err as Error).message}`) as Error & { code: string };
      fetchErr.code = 'URL_FETCH_FAILED';
      throw fetchErr;
    }

    const mediaType: MediaType = resolveMediaType(staging.content_type) ?? 'image/jpeg';
